# Getting Addition Adventure onto the App Store — submission runbook

Adapted from the sibling Multiplication Adventure's runbook, whose Phase 2 is
already live (`iOS 1.0 Waiting for Review` as of 2026-07-17, confirmed in
App Store Connect). Two phases. **Phase 1 = TestFlight** (get it to family
fast). **Phase 2 = public App Store** (permanent, searchable). Do Phase 1
first; Phase 2 reuses everything from Phase 1 plus screenshots and a fuller
review.

The app is already in good shape: paid developer account (team `6A4J2GTB6F`,
confirmed active — 5 other apps live in this ASC account), offline / no-data-
collection, privacy manifest now shipped (`PrivacyInfo.xcprivacy`), `1.0 (1)`,
**universal (iPhone + iPad)**, encryption-exempt flag set. Metadata drafts are
in [METADATA.md](METADATA.md); the privacy policy to host is
...
Pick from [METADATA.md](METADATA.md) (first choice: **Addition Adventure**,
matching the sibling's naming pattern). You'll type it when creating the app
record; App Store Connect tells you instantly if it's taken. Have a backup
ready.
...
2. Platform **iOS**; Name **[your chosen name]**; Primary language **English
   (U.S.)**; Bundle ID **`com.levelup.addsub`** (pick it from the dropdown —
   ...
   `addition-adventure-001`.

### 2. Upload the build from Xcode

The dev-signed builds used for the kids' iPads are *not* the same as an App
Store upload. Use Xcode's Organizer:
...
> Version/build: this batch is `1.0 (1)`. Every *new* upload needs a higher
> build number — bump `CURRENT_PROJECT_VERSION` in `project.yml` (1 → 2 → …)
> and run `xcodegen generate` before re-archiving. I can do that bump each time.
...
- **Individual emails:** TestFlight → **Internal Testing** (people on your dev
  ...
  to 10,000). For family use **External**: create a group, add their emails.
- **Public link:** in the External group, enable **Public Link** — you get a URL
  you can text to parents. They open it on the device, install **TestFlight**
...
review — usually < 24h). Submit the build for beta review; once approved,
invites work and the public link goes live. Internal testers can install
immediately with no review.

**That's it for Phase 1.** Push updates by bumping the build number,
re-archiving, and uploading — testers get them automatically. Builds expire
after 90 days; re-upload to refresh.
...
Since this app is **universal**, App Store requires BOTH:
- **13-inch iPad**: 2064×2752 portrait or 2752×2064 landscape.
...
- Primary **Education**, Secondary **Games** (subcategory **Family**) — matches
  the sibling's live setting exactly.
...
- Host the privacy policy once you tell me which of the three options in
  Section A above you want.
- Everything except the steps that need your Apple ID login (creating the
  record, clicking Upload/Submit, adding testers) — those are yours, or I can
  drive them in the browser once you say go for each one.
